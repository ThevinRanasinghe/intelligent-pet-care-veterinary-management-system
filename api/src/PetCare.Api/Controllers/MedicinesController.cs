...
public class MedicinesController : ControllerBase
{
    ...
    /// <summary>Gets all active batches that have already expired (expiryDate in the past).</summary>
    [Authorize(Roles = $"{Roles.ClinicManager},{Roles.InventoryOfficer},{Roles.SuperAdmin}")]
    [HttpGet("expired")]
    [ProducesResponseType(typeof(IReadOnlyList<MedicineBatchResponse>), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(StatusCodes.Status403Forbidden)]
    public async Task<ActionResult<IReadOnlyList<MedicineBatchResponse>>> GetExpiredStock(
        CancellationToken cancellationToken)
    {
        var batches = await _inventoryService.GetExpiredStockAsync(cancellationToken);
        return Ok(batches);
    }
