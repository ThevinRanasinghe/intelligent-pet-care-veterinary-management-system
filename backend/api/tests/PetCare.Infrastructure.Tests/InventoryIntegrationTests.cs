...
public class InventoryIntegrationTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    {
        Environment.SetEnvironmentVariable("PETCARE_DB_CONNECTION", DbConnection);
        Environment.SetEnvironmentVariable("PETCARE_JWT_KEY", TestJwtKey);

        _factory = factory.WithWebHostBuilder(builder =>
        {
            builder.UseSetting("ConnectionStrings:PetCareDb", DbConnection);
        ...
        _tokenGenerator = new JwtTokenGenerator(Options.Create(new JwtOptions
        {
            Key = TestJwtKey,
            Issuer = "PetCareApi",
    ...
    private string GenerateToken(string role, Guid? userId = null)
    {
        var user = new User
        {
            ...
            Email = $"{role.ToLower()}@test.lk",
            Name = $"Test {role}",
            Role = role,
            Active = true
        };
        return _tokenGenerator.GenerateToken(user).Token;
    }
