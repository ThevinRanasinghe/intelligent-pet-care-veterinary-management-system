...
export default function PetsPage() {
  ...
  async function loadData() {
    try {
      ...
      const [petsData, ownersData] = await Promise.all([
        petService.getAllPets().catch((err) => {
          console.error("Failed to load pets:", err);
          return [];
        }),
        ownerService.getAllOwners().catch((err) => {
  ...
  async function handlePetSubmit(event: React.FormEvent) {
    ...
    if (!petForm.ownerId.trim()) {
      setError("Please select or enter an Owner.");
      return;
    }
